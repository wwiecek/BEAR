...
Reference: for extraction package, see Schwab 2024
...
Reference: these two datasets come from the same replication package and shared data processing pipeline in Yang et al 2024. Original references for the datasets are Yang et al 2023 and Costello and Fox 2022
...
Reference: for R package, see Herold 2025
...
Reference: R package `metapsyData`: Harrer, Karyotaki, and Cuijpers 2022.
...
Reference: Rodriguez and Williams 2022 for R package `psymetadata` 
...
Notes: 
In the paper we disaggregate two datasets: Many Labs 2 project (Klein et al. 2018), which studied how replication results vary across labs and settings for a set of classic and contemporary effects in psychology, and a large meta-meta-analysis intelligence research (Nuijten et al. 2020). Remaining 20 datasets vary from 65 rows of data to over 1,000.

Data processing:
minimal. We used regular expressions to extract year labels from titles. We dropped some invalid rows (missing, non-finite, or non-positive standard errors). For "sala2019" ("on whether skills learned from cognitive training generalize to other situations") we can group by meta-analysis and for Many Labs we can group by experiment---we use these as `metaid` column.
