...
A large proportion of CDSR rows have binary outcomes with no events in both arms or all events in both arms. In the BEAR analysis subset before this exclusion, 2,371 of 26,619 binary rows (8.9%) had zero events in both arms or all events in both arms. Such rows can bias estimation of publication bias in metascientific investigations because they can lead to zero-inflated distributions of test statistics. They remain available in the processed source file `data/Cochrane.rds`, but are excluded from the main analysis dataset `BEAR.rds`.
