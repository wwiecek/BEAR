...
\urlstyle{same}
\hypersetup{
  pdftitle={Datasets used in the paper},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Datasets used in the paper}
\usepackage{etoolbox}
\makeatletter
...
\textbf{Reference:} A set of psychology datasets compiled by Rodriguez
and Williams (2022). We also disaggregate a large meta-meta-analysis of
intelligence research (Nuijten et al. 2020).
...
psychology for teaching and demonstrations. Nuijten et al. (2020) is a
large meta-meta-analysis of intelligence research, which focused on
power and small study effects.
...
\textbf{Reference:} Askarov et al. (2023).
...
\texttt{metapsyData} (Harrer et al. 2022). The Metapsy FAQ states that
all data provided as part of the project are open access and that
attribution is required. The source contains RCTs of psychotherapeutic
interventions.
...
\textbf{Reference:} Arel-Bundock et al. (2022).
...
(Yang et al. 2024). The original references for the datasets are Yang et
al.~2023 and Costello and Fox 2022 (Yang et al. 2023; Costello and Fox
2022).
...
Education Sciences (Institute of Education Sciences 2025).
...
\url{https://www.cochranelibrary.com/} (Cochrane Collaboration 2025).

\textbf{Snapshot date:} 20 November 2025

\textbf{Data collection:} Using the R package \texttt{cochrane} (Schwab
2024), we combined data from 8,726 CDSR review records into a single
file, \texttt{cdsr\_interventions\_19nov2025.csv} (63 MB).
...
\textbf{Reference:} Chavalarias et al. (2016).
...
\textbf{Reference:} {Tyner et al.} (2026).
...
\textbf{Reference:} Szucs and Ioannidis (2017).
...
Trials that reported results at ClinicalTrials.gov (National Library of
Medicine 2025).
...
\url{https://www.clinicaltrialsregister.eu/} (European Medicines Agency
2025).

\textbf{Snapshot date:} 18 January 2026

\textbf{Data collection:} We used the R package \texttt{ctrdata} (Herold
2025) to create a snapshot of EU CTR (\texttt{collection\ =\ "euctr"});
we downloaded only trial records with results. We stored the downloaded
records in a local SQLite database and extracted a fixed set of protocol
and results fields plus several fields computed by \texttt{ctrdata}
(trial phase, sample size, and a ``first primary endpoint'' p-value/size
derived by the package). We use only the EU CTR portion of this
extraction in BEAR.
...
\textbf{Reference:} Barnett and Wren (2019).
...
\textbf{Data processing:} We set missing \texttt{ci.level} values to
0.95. Among intervals with a known confidence level, only about 0.3\%
are not 95\%. Rows excluded under the default CI procedure remain in the
individual dataset for future investigations.

We use the default approach to deriving estimates from confidence
intervals (see elsewhere in the documentation). Since all measures are
ratios, we use the log scale for all calculations. Using the interval
midpoint rather than the reported point estimate changes the significant
...
\textbf{Reference:} Open Science Collaboration (2015).
...
90\% confidence interval. If a confidence interval is present but its
level is missing, we assume 95\% by default. Some data state whether CIs
or p-values are one- or two-sided.
...
positive; we keep non-ratio measures on the raw scale. When constructing
\texttt{BEAR.rds} we drop rows where where the lower bound is higher
...
close to it), we bound the implied \(\alpha\) at a small value. After
conversion to the chosen scale, we compute the two half-widths around
the reported point estimate, convert each to a standard error, and use
their average when both are finite.
...
Moreover, we compute a simple symmetry diagnostic for the confidence
interval on the chosen scale, defined as the smaller of the two
half-widths divided by the larger. We treat intervals as ``symmetric
enough'' only if this ratio exceeds 0.8. Otherwise, we use a p-derived
z-statistic where one is available; if not, we drop the row from
\texttt{BEAR.rds}. The original values remain available in the
individual datasets for other investigations.

In the current version, this rule either selects p-values for about
5--10\% of eligible clinical-trial registry rows or removes CI-only
rows, including some Barnett--Wren intervals. A 0.95 cut-off would
remove or replace roughly twice as many registry intervals. The
implications depend on the analysis.
...
\bibitem[\citeproctext]{ref-arel2022quantitative}
Arel-Bundock, Vincent, Ryan C. Briggs, Hristos Doucouliagos, Marco
Mendoza Aviña, and Tom D. Stanley. 2022. {``Quantitative {Political
Science Research} Is {Greatly Underpowered}.''} \emph{I4R Discussion
Paper Series}, no. 6.

\bibitem[\citeproctext]{ref-askarov2023significance}
Askarov, Zohid, Anthony Doucouliagos, Hristos Doucouliagos, and T D
Stanley. 2023. {``The Significance of Data-Sharing Policy.''}
\emph{Journal of the European Economic Association} 21 (3): 1191--226.

\bibitem[\citeproctext]{ref-barnett2019examination}
Barnett, Adrian Gerard, and Jonathan D Wren. 2019. {``Examination of
{CIs} in Health and Medical Journals from 1976 to 2019: An Observational
Study.''} \emph{BMJ Open} 9 (11): e032506.

\bibitem[\citeproctext]{ref-chavalarias2016evolution}
Chavalarias, David, Joshua David Wallach, Alvin Ho Ting Li, and John P A
Ioannidis. 2016. {``Evolution of Reporting p Values in the Biomedical
Literature, 1990-2015.''} \emph{Journal of the American Medical
Association} 315 (11): 1141--48.

\bibitem[\citeproctext]{ref-cochrane2025cdsr}
Cochrane Collaboration. 2025. \emph{Cochrane Database of Systematic
Reviews}. \url{https://www.cochranelibrary.com/cdsr}.

\bibitem[\citeproctext]{ref-costello2022decline}
Costello, Laura, and Jeremy W Fox. 2022. {``Decline Effects Are Rare in
Ecology.''} \emph{Ecology} 103 (6): e3680.

\bibitem[\citeproctext]{ref-euctr2025}
European Medicines Agency. 2025. \emph{EU Clinical Trials Register}.
\url{https://www.clinicaltrialsregister.eu/}.

\bibitem[\citeproctext]{ref-harrer2022metapsydata}
Harrer, Mathias, Eirini Karyotaki, and Pim Cuijpers. 2022.
\emph{metapsyData: Meta-Analytic Psychotherapy Databases}.
\url{https://CRAN.R-project.org/package=metapsyData}.

\bibitem[\citeproctext]{ref-herold2025ctrdata}
Herold, R. 2025. {``Aggregating and Analysing Clinical Trials Data from
Multiple Public Registers Using r Package Ctrdata.''} \emph{Research
Synthesis Methods}, 1--33. \url{https://doi.org/10.1017/rsm.2025.10061}.

\bibitem[\citeproctext]{ref-wwc2025studyfindings}
Institute of Education Sciences. 2025. \emph{What Works Clearinghouse
Study Findings}. \url{https://ies.ed.gov/ncee/wwc/studyfindings}.

\bibitem[\citeproctext]{ref-lang2025credible}
Lang, Kevin. 2025. {``How Credible Is the Credibility Revolution?''}
\emph{Journal of Labor Economics} 43 (2): 635--63.
\url{https://doi.org/10.1086/732772}.

\bibitem[\citeproctext]{ref-clinicaltrials2025}
National Library of Medicine. 2025. \emph{ClinicalTrials.gov}.
\url{https://clinicaltrials.gov/}.

\bibitem[\citeproctext]{ref-nuijten2020intelligence}
Nuijten, Michèle B., Marcel A. L. M. Van Assen, Hilde E. M. Augusteijn,
Elise A. V. Crompvoets, and Jelte M. Wicherts. 2020. {``Effect {Sizes},
{Power}, and {Biases} in {Intelligence Research}: {A
Meta-Meta-Analysis}.''} \emph{Journal of Intelligence} 8 (4): 36.
\url{https://doi.org/10.3390/jintelligence8040036}.

\bibitem[\citeproctext]{ref-opensciencecollaborationEstimatingReproducibilityPsychological2015a}
...
\emph{Journal of Open Psychology Data}, ahead of print.
\url{https://doi.org/10.5334/jopd.61}.

\bibitem[\citeproctext]{ref-schwab2024cochrane}
Schwab, Simon. 2024. \emph{Cochrane: Import Data from the Cochrane
...
\bibitem[\citeproctext]{ref-tyner2026investigating}
{Tyner, Andrew H., Anna Lou Abatayo, Mason Daley, et al.} 2026.
{``Investigating the Replicability of the Social and Behavioural
Sciences.''} \emph{Nature} 652: 143--50.
\url{https://doi.org/10.1038/s41586-025-10078-y}.

\bibitem[\citeproctext]{ref-yang2023publication}
Yang, Yefeng, Alfredo Sánchez-Tójar, Rose E O'Dea, et al. 2023.
{``Publication Bias Impacts on Effect Size, Statistical Power, and
Magnitude (Type {M}) and Sign (Type {S}) Errors in Ecology and
Evolutionary Biology.''} \emph{BMC Biology} 21 (1): 71.

\bibitem[\citeproctext]{ref-yang2024large}
Yang, Yefeng, Erik van Zwet, Nikolaos Ignatiadis, and Shinichi Nakagawa.
2024. {``A Large-Scale in Silico Replication of Ecological and
Evolutionary Studies.''} \emph{Nature Ecology \& Evolution}, 1--5.
