...
\subsection{Note on data cleaning and default imputed
values}\label{note-on-data-cleaning-and-default-imputed-values}
...
lower bound, upper bound, and point estimate were all strictly positive)
and non-ratio measures were always kept on the raw scale.

To derive SEs from confidence intervals, we first computed the critical
value. To avoid undefined critical values (e.g.~when at 100\% or
extremely close to it), we bounded the implied \(\alpha\) at a small
value. We then computed two one-sided standard error estimates from the
upper and lower bounds and used their average when both were finite.
...
ratio exceeded 0.8. This arbitrary threshold affects when we use the
CI-derived z-statistic; otherwise, we use a p-derived z-statistic where
one is available.

In datasets where additional information is present, we look for
potentially non-Wald intervals by matching text in the column describing
the parameter type. We search for keywords such as median, Hodges,
posterior/Bayes, exact, Fieller, bootstrap, and permutation. For
estimates that are flagged by this we prefer to derive z-value from
p-value,
...
sometimes find ambiguity in how p-values were recorded. Some significant
results have reported p-values close to 1, suggesting that authors
reported the cumulative distribution, \(\Phi(z)\). Therefore, for each
row we also calculated \(2\cdot\min\{p,1-p\}\). When a CI-derived
z-value was available, we chose whichever p-value led to a z-value that
was closest to the CI-derived z.

To avoid dropping rows, we retain unsigned z-statistics when the sign
could not be determined. If the sign of the effect was known, we applied
it to the p-derived z; otherwise we used its absolute value.
...
standard error, (3) not flagged as non-Wald by keyword search in the
measure label (see above), and (4) either missing symmetry information
or symmetry above 0.8. Otherwise we used the p-derived z.
...
\(|z|\). This inequality is recorded to BEAR. Importantly for some
future modelling, the operator in BEAR data applies to the magnitude,
including when the stored z-value is negative. For example, a stored
value of \(-1.96\) with operator \texttt{\textgreater{}} means a
negative direction and magnitude greater than 1.96, not \(z>-1.96\).

Exact zero p-values can produce infinite z-values. We retain infinities
in BEAR source data, although note that in modelling signal-to-noise
ratios these infinite values are truncated.
