...
But this should not be taken as a negative verdict on statistically-based science as a whole. To study this, we have created an extensive open-source database of empirical research results and focus here on 16 publicly available corpora in diverse fields.  Our optimistic finding is that, in these corpora, most results seem to be correct in sign.  Our remaining pessimism from selection in analysis and reporting of results, which points toward the need for real replication---not in the hope of confirming significance but to understand effects, and to minimize selection in all phases of the process.

On one hand, people know that you shouldn't rely on a single study; replication and, ultimately, meta-analysis are the way to go.  But then people are dismayed when results fail to replicate.  One point of the present paper is that apparent replication rates can be low even when studies are finding true effects, hence apparent low replication rates should not be taken as implying that most of science is wrong.  But selection.   We do this using a combination of theoretical and empirical analysis.  Medical reversal \parencite{prasad2011}.
...
Figure~\ref{fig:zmixtures} displays the histograms of the observed absolute $z$-statistics of our 16 corpora. The solid curves are the fitted mixtures of half-normal distributions conditional on publication, and the dashed curves are the ``corrected'' unconditional distributions. We find that the estimated distributions generally track the histogram well and conclude that our model is appropriate for our purpose of quantifying large-scale properties of research areas. The first eight panels are curated evidence collections and the second eight are domain samples and replication projects, arranged within groups by their estimated assurances $\overline{\text{PoS}}$. 
...
\caption{Histograms of absolute $z$-values across 16 corpora with fitted curves overlaid on top. The solid curves are densities, fitted using mixtures of half-normal distributions and conditioning on publication. Dashed curves are the unconditional ``corrected'' distributions, scaled to match the solid curve in region $|z| > 1.96$. Blue: curated evidence collections; purple: domain samples; brown: replication projects.}\label{fig:zmixtures}
\end{figure}
...
\vspace{-.2in}
\caption{Left panel: ``survival curves'' of the probability of significance: $1 - (\mbox{CDF of PoS})$ for each of the 16 corpora. Right panel: conditional probabilities that the observed direction of the effect matches the true direction (top set of curves) and the conditional probability of ``successful'' replication (bottom set). Colors here are the same as in Figure~\ref{fig:zmixtures}.}\label{fig:pos_gap}
\end{figure}

The estimated relative probability of reporting, $\hat \omega$, can be as low as 0.08. However, Figure~\ref{fig:zmixtures} highlights a clear divide: we find much less selection on significance in the curated datasets, with $\hat \omega > 0.66$ across all eight sets, than in the domain-representative samples (excluding two replication sets), where $\hat \omega$ ranges from 0.08 \textcite{barnett2019examination} for to 0.50 for clinical trial registries. However, we must be careful not to over-interpret this result. The corpus of \textcite{barnett2019examination} is based on confidence intervals, and it is possible that non-significant results were actually reported, just not with the associated confidence intervals. 
